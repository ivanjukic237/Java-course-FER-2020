...
/**
 * This class does ray tracing for a scene with some objects in it and animates
 * the movement of the observer. Ray tracing creates a model for determining a
 * 2D picture for what the observer sees while observing a scene of 3D objects
 * and some number of light sources. Algorithm first finds the 2D picture of the
 * scene observer sees, and then colors every pixel with linear combination of
 * diffuse, ambient and reflective intensities of light sources whose rays
 * intersect those objects. This class uses the ForkJoinPool framework to make
 * the coloring of every pixel parallel.
 * 
 * @author Ivan Jukić
 *
 */
...
public class RayCasterParallel2 {
	...
	/**
	 * Runs the ray caster and animator.
	 * 
	...
	private static IRayTracerAnimator getIRayTracerAnimator() {
		return new IRayTracerAnimator() {
			...
			public long getTargetTimeFrameDuration() {
				return 200; // redraw scene each 150 milliseconds
			}
